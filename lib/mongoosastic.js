...
module.exports = function Mongoosastic(schema, options){
  ...
  */
  ...
  schema.methods.index = function(options, cb){
	...
	serialize(this, mapping, function(err, doc) {
		...
		body: {
			doc: doc,
			accountPath: this.accountPath && this.accountPath.replace('/', ',')
		}
	      };
		if(parent) message.parent = parent && this[parent] && this[parent].toString();
		esClient.index(message, cb)
...
function createMappingIfNotPresent(options, cb) {
  ...
  generator.generateMapping(schema, function(err, mapping) {
	...
	if(options.parent) mapping._parent = { type: options.parent };
    var completeMapping = {}
